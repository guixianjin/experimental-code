...
""" pairwise label noise CIFAR-100 """
...
class PN_cifar100_revised_trainset(data.Dataset):  
    """ pairwise label noise train set """
...
class part_PN_cifar100_revised_trainset(data.Dataset):  # use it to get selected data
...
def split_raw_trainset(raw_trainset, noise): # a strafyied sampling
    ...
    # circulant noise label
    noise_label = [(x+1)%100 for x in range(100)]  # 100 classes
    ...
    for i, (_, label) in enumerate(raw_trainset):
        ...
        if np.random.rand() > noise:
            flip_labels[i] = label
        else: # filp to any class
            flip_labels[i] = noise_label[label]
...
def get_dataset(noise_rate):
    ...
    my_trainloader = torch.utils.data.DataLoader(
        PN_cifar100_revised_trainset(raw_trainset, train_ind, flip_labels),
        batch_size = 128,
        shuffle = True,
        )
...
def get_part_data(part_train_ind, label_50000, need_test_loader=False):
    ...
    part_trainloader = torch.utils.data.DataLoader(
        part_PN_cifar100_revised_trainset(raw_trainset, part_train_ind, label_50000),
        batch_size = 128,
        shuffle = True,
        )
    ...
    if not need_test_loader:
        ...
        part_trainloader_test = torch.utils.data.DataLoader(
            part_PN_cifar100_revised_trainset(raw_trainset_test, part_train_ind, label_50000),
            batch_size = 128,
            shuffle = True,
            )
