...
class UN_cifar10_revised_trainset(data.Dataset):  
...
class part_UN_cifar10_revised_trainset(data.Dataset):  # use it to get selected data
...
def split_raw_trainset(raw_trainset, noise): # a strafyied sampling
    ...
    for i, (_, label) in enumerate(raw_trainset):
        ...
        if np.random.rand() > noise:
            ...
            flip_labels[i] = np.random.randint(0, 10)
...
def get_dataset(noise_rate):
    ...
    my_trainloader = torch.utils.data.DataLoader(
        UN_cifar10_revised_trainset(raw_trainset, train_ind, flip_labels),
        batch_size = 128,
        shuffle = True,
        )
...
def get_part_data(part_train_ind, label_50000, need_test_loader=False):
    ...
    part_trainloader = torch.utils.data.DataLoader(
        part_UN_cifar10_revised_trainset(raw_trainset, part_train_ind, label_50000),
        batch_size = 128,
        shuffle = True,
        )
    ...
    if need_test_loader:
        ...
        part_trainloader_test = torch.utils.data.DataLoader(
            part_UN_cifar10_revised_trainset(raw_trainset_test, part_train_ind, label_50000),
            batch_size = 128,
            shuffle = True,
            )
